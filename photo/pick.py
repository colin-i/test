...
class picker(Gtk.Window):
    ...
    def button_press(self,widget,data):
        ...
        px=cenpx.get_pixels()[:3]
        #color=tuple(map(ord, px))
        c0=px[0]
        c1=px[1]
        c2=px[2]
        #col=Gdk.RGBA(float(color[0])/256.,float(color[1])/256.,float(color[2])/256.)
        col=Gdk.RGBA(float(c0)/256.,float(c1)/256.,float(c2)/256.)
...
#alternatives: xmag(also has -mag),gpick,gimp,shutter(uses imagemagick,not tested),colorpix(small windows wine,not tested)
